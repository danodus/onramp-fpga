...
static bool sdc_write(uint32_t first_block_addr, uint8_t* buf, size_t nb_bytes) {
    size_t remaining_bytes;
    uint32_t block_addr;
    size_t s, i;
    uint8_t b[SDC_BLOCK_LEN];
...
static bool read_fat(fs_fat_t* fat) {
    if (!sdc_read(FS_PARTITION_BLOCK_ADDR, (uint8_t*)fat, sizeof(fs_fat_t)))
        return false;
...
static bool write_fat(const fs_fat_t* fat) {
    if (!sdc_write(FS_PARTITION_BLOCK_ADDR, (uint8_t*)fat, sizeof(fs_fat_t)))
        return false;
    return true;
}
...
static void remove_file_blocks(fs_fat_t* fat, fs_file_info_t* file_info, size_t from_offset) {
    // erase block entries
    uint16_t block_table_index = file_info->first_block_table_index;
    uint32_t nb_blocks = (file_info->size + SDC_BLOCK_LEN - 1) / SDC_BLOCK_LEN;
    size_t offset = SDC_BLOCK_LEN;
...
bool fs_format(fs_context_t* ctx, bool quick) {
    if (!quick) {
        ...
        for (uint32_t i = 0; i < nb_partition_blocks; ++i) {
            PRINT_DBG(".");
            if (!sdc_write_single_block(FS_PARTITION_BLOCK_ADDR + i, buf)) {
...
static bool fs_read_a(const fs_file_info_t* file_info, const fs_fat_t* fat, uint8_t* buf, size_t current_pos, size_t nb_bytes, size_t* nb_read_bytes) {
    ...
    while (block_table_index != 0xFFFF && remaining_bytes > 0) {
        ...
        if (remaining_bytes <= nb_bytes) {
            ...
            if (!sdc_read_single_block(first_block_addr + block_addr, b)) {
...
static bool fs_write_a(fs_file_info_t* file_info, const fs_fat_t* fat, fs_fat_t* tmp_fat, const uint8_t* buf, size_t current_pos, size_t nb_bytes) {
    ...
    while (remaining_bytes > 0) {
        ...
        if (!sdc_write_single_block(first_block_addr + block_addr, b)) {
